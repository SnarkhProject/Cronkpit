    ...
    class Floor
    {
        ...
        public void decay_all_scents()
        {
            for (int x = 0; x < stdfloorSize; x++)
                for (int y = 0; y < stdfloorSize; y++)
                {
                    //if (floorTiles[x][y].strength_of_scent(0) > 0)
                        //add_new_popup(floorTiles[x][y].strength_of_scent(0).ToString(), Popup.popup_msg_color.Yellow, new gridCoordinate(x, y));
                    floorTiles[x][y].decayScents();
                }
        }

        //Green text.
        public void scent_pulse_raycast(gridCoordinate origin, int targetSmell, int monsterID, int smellRange, int smellThreshold)
        {
            ...
            while (Vision_Rc.Count > 0)
            {
                for (int i = 0; i < Vision_Rc.Count; i++)
                {
                    ...
                    if (current_smell > strongest_smell_value && current_smell > smellThreshold)
                    {
                        ...
                        for (int j = 0; j < badGuys.Count; j++)
                        {
                            if (monsterID == badGuys[j].my_Index)
                            {
                                badGuys[j].has_scent = true;
                                badGuys[j].strongest_smell_coord.x = xPosition;
                                badGuys[j].strongest_smell_coord.y = yPosition;
                            }
                        }
                    }
        ...
        {
            int max_coord = stdfloorSize - 1;
            for (int x = my_grid_coord.x - radius; x <= my_grid_coord.x + radius; x++)
                for (int y = my_grid_coord.y - radius; y <= my_grid_coord.y + radius; y++)
                {
                    int safe_x_val = Math.Max(0, Math.Min(x, max_coord));
                    int safe_y_val = Math.Max(0, Math.Min(y, max_coord));
                    if (floorTiles[safe_x_val][safe_y_val].strength_of_scent(targetSmell) >= smell_threshold)
                        return true;
                }
